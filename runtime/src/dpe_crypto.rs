...
use crypto::{
    ecdsa::{
        curve_256::EcdsaSignature256,
        curve_384::{EcdsaPub384, EcdsaSignature384},
        EcdsaPubKey, EcdsaSignature,
    },
    Crypto, CryptoBuf, CryptoError, Digest, DpeProfile, ExportedPubKey, Hasher, Signature,
    SignatureAlgorithm,
};
...
impl DpeProfile for DpeCrypto<'_> {
    // TODO(clundin): Make it so I can call the const function.
...
impl Hasher for DpeHasher<'_> {
    ...
    fn finish(self) -> Result<Digest, CryptoError> {
        ...
        self.op
            .finalize(&mut digest)
            .map_err(|e| CryptoError::HashError(u32::from(e)))?;
        Digest::new(<[u8; EcdsaAlgorithm::Bit384.curve_size()]>::from(digest).as_ref())
    }
}
...
impl Crypto for DpeCrypto<'_> {
    ...
    fn sign_with_derived(
        ...
        let mut digest_arr = [0u8; Self::SIGNATURE_ALGORITHM.digest_size()];
        digest_arr
            .get_mut(..Self::SIGNATURE_ALGORITHM.digest_size())
            .ok_or(CryptoError::CryptoLibError(0))?
            .copy_from_slice(
                digest
                    .bytes()
                    .get(..Self::SIGNATURE_ALGORITHM.digest_size())
                    .ok_or(CryptoError::CryptoLibError(0))?,
            );
        ...
        let s = &<[u8; EcdsaAlgorithm::Bit384.curve_size()]>::from(&sig.r);

        Ok(Signature::Ecdsa(EcdsaSignature::Ecdsa384(
            EcdsaSignature384::from_slice(r, s).unwrap(),
        )))
    }
