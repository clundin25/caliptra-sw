...
use dpe::{response::DpeErrorCode, x509::MeasurementData, MAX_EXPORTED_CDI_SIZE};
...
impl<'a> Crypto for DpeCrypto<'a> {
    ...
    fn derive_exported_cdi(
        ...
        self.derive_cdi_inner(algs, measurement, info, KEY_ID_EXPORTED_DPE_CDI)
    }
    ...
    fn get_exported_cdi_handle(
        ...
        for slot in self.exported_cdi_slots.iter_mut() {
            match slot {
                ...
                Some((cached_cdi, handle)) if cached_cdi == cdi => return Ok(*handle),
                // Empty slot
                None => {
