...
fn timeout_to_mtime(
    ready_timeout: Millisecond,
    command_timeout: Millisecond,
    clock_period: Picosecond,
) -> (u64, u64) {
    ...
    let bounded_ready_timeout: Millisecond = ready_timeout.min(MAX_TIMEOUT);
    ...
    let ready_mtimeout = walltime_to_mtime(bounded_ready_timeout, period);
    let command_mtimeout = walltime_to_mtime(bounded_command_timeout, period);
