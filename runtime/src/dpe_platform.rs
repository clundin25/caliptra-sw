...
impl Platform for DpePlatform<'_> {
    ...
    fn get_issuer_name(
        ...
        for &b in self.hashed_rt_pub_key.as_bytes() {
            let h1 = (b >> 4) as usize;
            ...
            {
                return Err(PlatformError::PrintError(0));
            }
    ...
    fn get_signer_identifier(&mut self) -> Result<SignerIdentifier, PlatformError> {
        ...
        let hashed_rt_pub_key = self.hashed_rt_pub_key.as_bytes();
