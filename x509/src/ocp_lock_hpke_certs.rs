...
pub mod ml_kem_1024_ecdh_384_ecc_348 {

    #[cfg(feature = "generate_templates")]
    include!(concat!(
        env!("OUT_DIR"),
        "/ocp_lock_ml_kem_1024_ecdh_384_cert_tbs_ecc_384.rs"
    ));
    #[cfg(not(feature = "generate_templates"))]
    include! {"../build/ocp_lock_ml_kem_1024_ecdh_384_cert_tbs_ecc_384.rs"}
    ...
    mod tests {
        ...
        fn test_cert() {
            ...
            let issuer_key = Ecc384AsymKey::default();
            let ec_key = issuer_key.priv_key().ec_key().unwrap();
            let params = OcpLockMLKem1024Ecdh384CertTbsEcc384Params {
                serial_number: &[0xABu8; OcpLockMLKem1024Ecdh384CertTbsEcc384Params::SERIAL_NUMBER_LEN],
                public_key:
                    TryInto::<&[u8; OcpLockMLKem1024Ecdh384CertTbsEcc384Params::PUBLIC_KEY_LEN]>::try_into(
                        subject_key.pub_key(),
                    )
                    .unwrap(),
                subject_sn:
                    &TryInto::<[u8; OcpLockMLKem1024Ecdh384CertTbsEcc384Params::SUBJECT_SN_LEN]>::try_into(
                        subject_key.hex_str().into_bytes(),
                    )
                    .unwrap(),
                issuer_sn:
                    &TryInto::<[u8; OcpLockMLKem1024Ecdh384CertTbsEcc384Params::ISSUER_SN_LEN]>::try_into(
                        issuer_key.hex_str().into_bytes(),
                    )
                    .unwrap(),
                subject_key_id: &TryInto::<
                    [u8; OcpLockMLKem1024Ecdh384CertTbsEcc384Params::SUBJECT_KEY_ID_LEN],
                >::try_into(subject_key.sha1())
                .unwrap(),
                authority_key_id: &TryInto::<
                    [u8; OcpLockMLKem1024Ecdh384CertTbsEcc384Params::AUTHORITY_KEY_ID_LEN],
                >::try_into(issuer_key.sha1())
                .unwrap(),
                not_before: &NotBefore::default().value,
                not_after: &NotAfter::default().value,
            ...
            let cert = OcpLockMLKem1024Ecdh384CertTbsEcc384::new(&params);
            ...
            assert_ne!(cert.tbs(), OcpLockMLKem1024Ecdh384CertTbsEcc384::TBS_TEMPLATE);
            assert_eq!(
                &cert.tbs()[OcpLockMLKem1024Ecdh384CertTbsEcc384::PUBLIC_KEY_OFFSET
                    ..OcpLockMLKem1024Ecdh384CertTbsEcc384::PUBLIC_KEY_OFFSET
                        + OcpLockMLKem1024Ecdh384CertTbsEcc384::PUBLIC_KEY_LEN],
                params.public_key,
            );
            assert_eq!(
                &cert.tbs()[OcpLockMLKem1024Ecdh384CertTbsEcc384::SUBJECT_SN_OFFSET
                    ..OcpLockMLKem1024Ecdh384CertTbsEcc384::SUBJECT_SN_OFFSET
                        + OcpLockMLKem1024Ecdh384CertTbsEcc384::SUBJECT_SN_LEN],
                params.subject_sn,
            );
            assert_eq!(
                &cert.tbs()[OcpLockMLKem1024Ecdh384CertTbsEcc384::ISSUER_SN_OFFSET
                    ..OcpLockMLKem1024Ecdh384CertTbsEcc384::ISSUER_SN_OFFSET
                        + OcpLockMLKem1024Ecdh384CertTbsEcc384::ISSUER_SN_LEN],
                params.issuer_sn,
            );
            assert_eq!(
                &cert.tbs()[OcpLockMLKem1024Ecdh384CertTbsEcc384::SUBJECT_KEY_ID_OFFSET
                    ..OcpLockMLKem1024Ecdh384CertTbsEcc384::SUBJECT_KEY_ID_OFFSET
                        + OcpLockMLKem1024Ecdh384CertTbsEcc384::SUBJECT_KEY_ID_LEN],
                params.subject_key_id,
            );
            assert_eq!(
                &cert.tbs()[OcpLockMLKem1024Ecdh384CertTbsEcc384::AUTHORITY_KEY_ID_OFFSET
                    ..OcpLockMLKem1024Ecdh384CertTbsEcc384::AUTHORITY_KEY_ID_OFFSET
                        + OcpLockMLKem1024Ecdh384CertTbsEcc384::AUTHORITY_KEY_ID_LEN],
                params.authority_key_id,
            );
            ...
            std::fs::write("cert_hybrid.der", &buf);
        ...
        fn test_ocp_lock_ecdh_ecc384_template() {
            let manual_template = std::fs::read(std::path::Path::new(
                "./build/ocp_lock_ml_kem_1024_ecdh_384_cert_tbs_ecc_384.rs",
            ))
            .unwrap();
            let auto_generated_template = std::fs::read(std::path::Path::new(concat!(
                env!("OUT_DIR"),
