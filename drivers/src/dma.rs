...
impl<'a> DmaEncryptionEngine<'a> {
    ...
    fn wait_until_or_timeout<T1, T2>(
        &self,
        soc_ifc: &SocIfc,
        start_mtime: u64,
        timeout_mtime: u64,
        predicate_with_witness: impl Fn(&Self) -> (bool, T1),
        output: impl Fn(T1) -> CaliptraResult<T2>,
    ) -> CaliptraResult<T2> {
        loop {
            ...
            if let (true, x) = predicate_with_witness(self) {
                return output(x);
            }
        }
    }
    ...
    pub fn wait_ready(
        ...
        self.wait_until_or_timeout(
            soc_ifc,
            start_mtime,
            timeout_mtime,
            |x| {
                let ctrl = EncryptionEngineCtrl(x.read_ctrl());
                (ctrl.ready_bit(), ())
            },
            |_| Ok(()),
        )
    ...
    pub fn wait_done(
        ...
        self.wait_until_or_timeout(
            ...
            |x| {
                let ctrl = EncryptionEngineCtrl(x.read_ctrl());
                (ctrl.done_bit(), ctrl.0)
            },
            |v| Ok(Self::check_error(v)),
        )
    }
}
