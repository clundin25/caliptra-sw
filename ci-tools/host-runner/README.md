...
### Optionally add wifi access

If ethernet is unavailable, it's convenient to load a wifi configuration so you can ssh to the host runner.
...
```nix
  networking.wireless = {
    networks = {
      "my-network" = {
        # psk stored in a file. Remember, these files are world readable on the NixOS system!
        # If this is an issue, you should provision the wifi PSK differently.
        # Moving the PSK to a separate file is a convenience to avoid committing a PSK.
        psk = lib.strings.trim (builtins.readFile ./secrets/my-network/wifi-psk);
      };
    };
  };
```
...
```bash
# non-NixOS systems will need the NixOS build toolchain.
nix-shell -p nixos-rebuild
# NixOS already has the `nixos-rebuild` tool.
nixos-rebuild --target-host caliptra-hostrunner0 --use-remote-sudo switch --flake path:.#caliptra-hostrunner0 --impure # "caliptra-hostrunner0" is an SSH Host in this example.
```
 ...
 * `LOCATION`
 ...
 IMAGE="$HOME/zcu104.img"
 
caliptra-fpga-boss --zcu104 $ZCU_FTDI --sdwire $ZCU_SDWIRE serve $IMAGE -- cred-tool --stage prod --fpga-target zcu104 --fpga-identifier $IDENTIFIER --location $LOCATION --key-path /etc/secrets/caliptra-gce-ci-github-private-key-pem/prod
 ```
 
 ### Populating ZCU_FTDI
 
 This is alternatively documented at ["How do I determine the `--zcu104` parameter for my hardware?"](https://github.com/chipsalliance/caliptra-sw/tree/main/ci-tools/fpga-boss#how-do-i-determine-the---zcu104-parameter-for-my-hardware). We will use a different method because we will most likely be connecting multiple FPGAs.
 ...
 This is alternatively documented at ["How do I determine the `--sdwire` parameter for my hardware?"](https://github.com/chipsalliance/caliptra-sw/tree/main/ci-tools/fpga-boss#how-do-i-determine-the---sdwire-parameter-for-my-hardware). We will use a different method because we will most likely be connecting multiple FPGAs.
 ...
 You should define this in Nix using the pattern in `hostrunners/kir-0.nix`.
 
 ## Starting the FPGA service

 After rebuilding the hostrunner you should have the systemd services. Check that they are working by looking at the service logs.
